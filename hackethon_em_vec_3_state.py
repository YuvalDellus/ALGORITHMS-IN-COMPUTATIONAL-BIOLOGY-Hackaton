...
def calc_parameters(transitions, ph, fasta, ll_history, plot_mode_only=False):
    ...
    for from_state in range(3):
        for to_state in range(3):
            est_transitions[from_state, to_state] = np.sum(
                np.exp(forward_table[from_state, :-1] + transitions[from_state, to_state] +
...
if __name__ == '__main__':
    filename = "C:\\Users\yuval\Desktop\Liver-Hepatocytes-Z000000R3.beta"
    fasta = np.fromfile(filename, dtype=np.uint8).reshape((-1, 2))
    fasta = fasta[1166000:11167100, :]
    fasta = eliminate_zeros(fasta)
    transitions = np.array([[0.88, 0.04, 0.08], [0.08, 0.8, 0.1],
                            [0.75, 0.1, 0.15]])  # [[hth, htl, hto], [lth, ltl, lto], [oth, otl, oto]]
